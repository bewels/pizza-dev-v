...
export const basketReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case 'DEL':
            ...
            if (clearMenuItems.find(item => item._id === action.payload).count === 1){
                clearMenuItems = clearMenuItems.filter(item => item._id !== action.payload)
            } else {
                clearMenuItems.map(item => item._id === action.payload ? item.count-- : item)
            }
